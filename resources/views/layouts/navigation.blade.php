<nav x-data="{ open: false }" class="bg-white border-b border-gray-100" style="border-bottom-color: var(--primary-color, #e5e7eb);">
            ...
            <div class="flex">
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('tenant.dashboard', ['tenant' => tenant()?->subdomain ?? '']) }}">
                        @if(tenant()?->logo_path)
                            <img src="{{ asset('storage/' . tenant()->logo_path) }}" alt="{{ tenant()->name }}" class="block h-9 w-auto">
                        @else
                            <span class="text-lg font-bold" style="color: var(--primary-color);">{{ tenant()?->name ?? config('app.name') }}</span>
                        @endif
                    </a>
                </div>
