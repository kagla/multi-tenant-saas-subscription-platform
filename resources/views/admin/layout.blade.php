...
<head>
    ...
    <title>관리자 - {{ config('app.name') }}</title>
    ...
    <div class="min-h-screen flex">
        ...
        <aside class="w-64 bg-gray-900 text-white flex flex-col shrink-0">
            <div class="p-4 border-b border-gray-700">
                <h1 class="text-lg font-bold">슈퍼 관리자</h1>
        ...
        <div class="flex-1 flex flex-col min-w-0">
            <header class="bg-white shadow-sm border-b border-gray-200 px-6 py-4">
                <h2 class="text-xl font-semibold text-gray-800">@yield('title', '대시보드')</h2>
            </header>
            <main class="flex-1 p-6 overflow-auto">
                @if(session('status'))
                    <div class="mb-4 p-4 bg-green-50 border border-green-200 text-green-700 rounded-lg">
                        {{ session('status') === 'tenant-suspended' ? '테넌트가 정지되었습니다.' : (session('status') === 'tenant-activated' ? '테넌트가 활성화되었습니다.' : session('status')) }}
                    </div>
                @endif
                @if($errors->any())
