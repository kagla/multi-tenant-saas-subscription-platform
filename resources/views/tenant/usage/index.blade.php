<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Usage Dashboard') }}</h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                    ...
                    <div class="overflow-hidden shadow-sm sm:rounded-lg p-6 border {{ $bgColor }}">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            @if(!$isUnlimited && $metric['percent'] >= 80)
                                <span class="text-xs font-semibold px-2 py-0.5 rounded-full {{ $metric['percent'] >= 100 ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700' }}">
                                    {{ $metric['percent'] >= 100 ? 'Limit reached' : 'Nearing limit' }}
                                </span>
                            @endif
                        </div>
                        <div class="text-2xl font-bold text-gray-900">
                            ...
                            <span class="text-sm font-normal text-gray-500">
                                / {{ $isUnlimited ? 'Unlimited' : number_format($metric['limit']) . $unit }}
                            </span>
                        </div>
                        @if(!$isUnlimited)
                            ...
                            <p class="mt-1 text-xs text-gray-500">
                                {{ number_format($metric['remaining'], $unit === 'MB' ? 1 : 0) }}{{ $unit }} remaining
                            </p>
                        @else
                            <div class="mt-3 w-full bg-green-100 rounded-full h-2.5">
                                <div class="bg-green-400 h-2.5 rounded-full" style="width: 5%"></div>
                            </div>
                            <p class="mt-1 text-xs text-green-600">Unlimited on {{ ucfirst($tenant->plan) }} plan</p>
                        @endif
                    </div>
                @endforeach
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-medium text-gray-900">API Calls (Last 30 Days)</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    ...
                    <dl class="space-y-3">
                        ...
                        <div class="flex justify-between">
                            <dt class="text-sm text-gray-500">API Calls/Day</dt>
                            <dd class="text-sm font-semibold">
                                {{ $metrics['api_calls_per_day']['limit'] === PHP_INT_MAX ? 'Unlimited' : number_format($metrics['api_calls_per_day']['limit']) }}
                            </dd>
                        </div>
                        <div class="flex justify-between">
                        ...
                        <div class="flex justify-between">
                            ...
                            <dd class="text-sm font-semibold">
                                {{ $metrics['members']['limit'] === PHP_INT_MAX ? 'Unlimited' : $metrics['members']['limit'] }}
                            </dd>
                        </div>
                    </dl>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.getElementById('chart-total').textContent = `Total: ${total.toLocaleString()} calls`;
