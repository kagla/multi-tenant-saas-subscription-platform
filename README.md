...
1. 상단 메뉴 **결제** 클릭
2. **구독** 페이지에서 현재 플랜, 사용량, 기능 목록을 확인합니다.
3. **플랜 변경** 버튼 클릭 → 플랜 선택 페이지로 이동합니다.

##### 플랜 업그레이드 (KG이니시스)

1. 플랜 선택 페이지에서 원하는 플랜의 **체험 시작** 또는 **업그레이드** 버튼 클릭
2. KG이니시스 결제창이 팝업으로 표시됩니다 (카드/계좌이체/휴대폰결제 지원).
3. 결제 완료 후 자동으로 플랜이 업그레이드됩니다.
4. 정기결제(빌링키 방식): 매월 자동으로 결제가 진행됩니다.

> **테스트 모드**: MID `INIpayTest`로 실제 과금 없이 테스트 결제가 가능합니다.

##### 구독 취소/재개

- 결제 페이지에서 **구독 취소** → 현재 결제 기간(월말) 종료 후 Free로 전환
- 취소 후 종료 전까지 **구독 재개** 버튼으로 재개 가능
...
| 기능 | Free | Pro (33,000원/월) | Enterprise (110,000원/월) |
|---|:---:|:---:|:---:|
...
이 프로젝트는 **KG이니시스**(기본)와 **Stripe** 두 가지 PG를 지원합니다.
`.env`의 `PG_DRIVER` 값으로 전환합니다.

```env
PG_DRIVER=inicis   # KG이니시스 (기본값, 국내 결제)
PG_DRIVER=stripe   # Stripe (해외 결제)
```

### KG이니시스 설정 (기본 PG)

국내 PG 점유율 1위. 신용카드, 계좌이체, 가상계좌, 휴대폰결제, 카카오페이, 네이버페이를 지원합니다.

#### 결제 방식

| 방식 | 설명 | MID |
|---|---|---|
| **일반결제** | INIStd 표준결제창 (팝업) | `INIpayTest` |
| **정기결제** | 빌링키 발급 → 매월 자동 결제 | `INIBillTst` |
...
INICIS_API_KEY=ItEQKi3rY7uvDS8l
INICIS_IV=HYb3HAxk65TRbMni
INICIS_BILLING_MID=INIBillTst
```

> 위 값은 이니시스 테스트 키입니다. 실 운영 시 이니시스에서 발급받은 실제 MID/키로 교체합니다.

#### 결제 흐름

```
사용자 → 플랜 선택 페이지 → "체험 시작" 클릭
  → InicisController::billingCheckout()
  → inicis-checkout.blade.php (INIStd.js 결제창 팝업)
  → 카드 정보 입력 → 인증 완료
  → billingReturn() → 2단계 승인 요청 → 승인 성공
  → 빌링키 발급 + Subscription 레코드 저장 + 플랜 업그레이드
  → 구독 성공 페이지

[다음 달 자동 결제]
  → 스케줄러 → InicisService::billPayment(빌링키) → 자동 결제

[결제 취소]
  → InicisService::cancelPayment(TID) → 결제 취소 + 구독 종료
```

#### 노티 URL (서버간 결제 결과 통보)

```
POST https://tenant.gnuboard.net/webhook/inicis
```

이니시스 가맹점 관리자에서 노티 URL을 위 주소로 설정합니다.

#### 테스트

테스트 MID(`INIpayTest`)를 사용하면 실제 과금 없이 결제 테스트가 가능합니다. 테스트 결제창에서 아무 카드 정보를 입력하면 됩니다.

---

### Stripe 설정 (대안 PG)

해외 결제가 필요한 경우 Stripe를 사용할 수 있습니다.
...
1. [Stripe Dashboard](https://dashboard.stripe.com) → Products에서 상품 2개 생성:
   - **Pro Plan**: $29/month (Monthly recurring)
   - **Enterprise Plan**: $99/month (Monthly recurring)
2. 각 Price ID를 `.env`에 설정

#### Webhook 설정

1. Developers → Webhooks에서 엔드포인트 추가
2. URL: `https://tenant.gnuboard.net/webhook/stripe`
3. 이벤트: `checkout.session.completed`, `customer.subscription.updated`, `customer.subscription.deleted`, `invoice.payment_succeeded`, `invoice.payment_failed`

#### Stripe 테스트 카드

`4242 4242 4242 4242` (만료일: 아무 미래 날짜, CVC: 아무 3자리)
...
| `PG_DRIVER` | 결제 PG 선택 | `inicis` 또는 `stripe` |
| `INICIS_MID` | 이니시스 상점 아이디 | `INIpayTest` |
| `INICIS_SIGNKEY` | 이니시스 사인키 | `SU5JTElURV9...` |
| `INICIS_BILLING_MID` | 이니시스 빌링 MID (정기결제) | `INIBillTst` |
...
app/
  ...
  Services/
    InicisService            # KG이니시스 API (결제창, 승인, 빌링키, 취소)
...
resources/views/
  ...
  payment/                   # 이니시스 결제창 (inicis-checkout, inicis-close)
